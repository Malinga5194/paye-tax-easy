...
export default function EmployerSearchPage() {
  ...
  const search = async () => {
    ...
    setError('');
    try {
      const res = await axios.get(`${API}/ird-dashboard/search/employer/${tin.trim()}?financialYear=${fy}`, { headers });
      setResult(res.data);
    } catch (e: any) {
      if (e.response?.status === 404) {
        setNotFound(`No employer found with TIN ${tin.trim()}`);
      } else {
  ...
  const downloadPdf = async () => {
    ...
    try {
      const res = await axios.get(
        `${API}/ird-dashboard/search/employer/${result.employerTIN}/pdf?financialYear=${fy}`,
        { headers, responseType: 'blob' }
      );
      const url = URL.createObjectURL(new Blob([res.data]));
      const a = document.createElement('a');
      a.href = url;
      a.download = `IRD_Employer_Report_${result.employerTIN}_${fy}.pdf`;
      a.click();
    ...
    <div style={{ fontFamily: "'Segoe UI', sans-serif", background: '#f0f4f8', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '1.5rem', flex: 1, maxWidth: '800px', margin: '0 auto', width: '100%' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <h1 style={{ margin: 0, color: COLOR }}>🏢 Employer Search</h1>
            <p style={{ margin: '4px 0 0', color: '#666', fontSize: '0.9rem' }}>Search employers by Taxpayer Identification Number</p>
          </div>
          <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
            ...
            <button style={{ ...btn, background: '#6c757d' }} onClick={() => navigate('/employee-search')}>👤 Employee Search</button>
          </div>
        </div>
        ...
        <div style={sectionStyle}>
          <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem' }}>
            <input
              type="text"
              placeholder="Enter 9-digit Employer TIN"
          ...
          {result && (
            <>
              <div style={cardStyle}>
                <h3 style={{ margin: '0 0 0.75rem', color: '#003366' }}>{result.organizationName}</h3>
                <div style={gridStyle}>
                  <InfoRow label="TIN" value={result.employerTIN} />
                  <InfoRow label="Registration Number" value={result.registrationNumber} />
                  <InfoRow label="Contact Email" value={result.contactEmail} />
                  <InfoRow label="Employee Count" value={result.employeeCount} />
                  <InfoRow label="Total Submissions" value={result.totalSubmissions} />
                  <InfoRow label="Total PAYE Submitted" value={`Rs. ${result.totalPAYESubmitted.toLocaleString()}`} color="#003366" />
                  <InfoRow label="Latest Submission Ref" value={result.latestSubmissionRef || '—'} />
                  <InfoRow label="Latest Submission Date" value={result.latestSubmissionDate ? new Date(result.latestSubmissionDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : '—'} />
                </div>
              </div>
